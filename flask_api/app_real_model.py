                ...
                if original_col in input_data and original_col in model_package['label_encoders']:
                    try:
                        ...
                        if value in model_package['label_encoders'][original_col]['classes_']:
                            classes_list = model_package['label_encoders'][original_col]['classes_']
                            encoded_value = classes_list.index(value)
...
def predict_yield():
    ...
    try:
        ...
        if model_package.get('simulation_mode', False):
            ...
            logger.info(f"🎯 Simulation prediction result: {predicted_yield:.2f}")
        else:
            # Real model prediction
            predicted_yield = model_package['model'].predict(feature_array)[0]
            logger.info(f"🎯 Model prediction result: {predicted_yield:.2f}")
...
def get_model_info():
    ...
    try:
        ...
        for col_name, encoder in model_package['label_encoders'].items():
            categorical_features[col_name] = encoder['classes_']
